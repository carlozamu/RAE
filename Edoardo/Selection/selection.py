...
class RankBasedSelection(SelectionStrategy):
    ...
    def select(self, population: List[Dict[str, Any]], num_parents: int) -> List[Dict[str, Any]]:
        ...
        # Sort by fitness (ascending for minimization behavior)
        sorted_pop = sorted(population, key=lambda x: x['fitness'], reverse=False)
        n = len(sorted_pop)
...
class TournamentSelection(SelectionStrategy):
    ...
    def select(self, population: List[Dict[str, Any]], num_parents: int) -> List[Dict[str, Any]]:
        ...
        for _ in range(num_parents):
            ...
            # Select the best from the tournament (min fitness = best loss)
...
class FitnessProportionateSelection(SelectionStrategy):
    ...
    def select(self, population: List[Dict[str, Any]], num_parents: int) -> List[Dict[str, Any]]:
        ...
        # Handle minimization: Invert fitness (1/f) to probability
        # Add epsilon to prevent div/0
        inverted_fitnesses = [1.0 / (f + 1e-6) for f in fitnesses]
        total_inv_fitness = sum(inverted_fitnesses)
        
        if total_inv_fitness == 0:
             # If all infinite? Uniform
             probabilities = [1.0 / len(population)] * len(population)
        else:
            probabilities = [f / total_inv_fitness for f in inverted_fitnesses]
